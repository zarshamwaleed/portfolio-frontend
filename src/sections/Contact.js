  ...
  {contactChannels.map((channel, index) => {
    ...
    return isVoice ? (
      <div
        key={index}
        onClick={handleClick}
        className={`${channel.bg} p-6 rounded-xl border border-gray-200 dark:border-gray-800 hover:shadow-lg transition-all duration-300 hover:-translate-y-1 cursor-pointer`}
      >
  ...
  <div className="bg-gray-50 dark:bg-gray-900/50 rounded-xl p-6 border border-gray-200 dark:border-gray-800">
    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
      {[
        ...
        <div
          key={index}
          className="text-sm font-medium text-gray-800 dark:text-gray-300 bg-white/80 dark:bg-white/10 py-2 px-4 rounded-lg shadow-sm hover:scale-105 transition"
        >
          {tech}
        </div>
      ))}
