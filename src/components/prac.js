...
const ReviewPage = () => {
  ...
  const handleSubmitReview = async (e) => {
    ...
    try {
      const res = await axios.post('/api/reviews', reviewToPost);
  ...
  const handleHelpfulClick = async (id, helpfulCount) => {
    try {
      const res = await axios.patch(`/api/reviews/${id}/helpful`);
      setReviews((prev) =>
  ...
  return (
    <div
      ...
      style={{
        background: 'linear-gradient(to bottom right, rgb(240,240,240), rgb(200,200,200))',
      }}
    >
      {/* Header */}
